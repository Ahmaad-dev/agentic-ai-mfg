...
in turn requires a sys.path entry for `app/tools/smart-planning/runtime/`. Keeping that out
of the blueprint keeps the HTTP layer free of runtime imports and lets these functions
be tested without a Flask app.

Hard rule honoured here: runtime tools under `app/tools/smart-planning/runtime/` are NOT
...
_runtime_dir = str(Path(__file__).parent.parent / "tools" / "smart-planning" / "runtime")
