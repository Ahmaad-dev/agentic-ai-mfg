...
# .env Datei laden (aus dem Anwendungsverzeichnis app/)
try:
...
class SmartPlanningAPI:
    ...
    def __init__(self, base_uri: str = None):
        # `None` bedeutet „nimm die Umgebung" — ein ausdruecklich uebergebener Wert gewinnt.
        self.base_uri = (base_uri or SP_BASE_URI).rstrip("/")
        self.token_uri = f"{self.base_uri}/keycloak/realms/{SP_REALM}/protocol/openid-connect/token"
        self.api_base_uri = f"{self.base_uri}/esarom-be/api/v1"
        self.token: Optional[str] = None
        
    def authenticate(self, client_id: str = None, 
        ...
        data = {
            "client_id": client_id or SP_CLIENT_ID,
...
def download_and_save_snapshot(identifier: str, snapshots_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    Args:
        identifier: Snapshot-ID (UUID) oder Snapshot-Name
        snapshots_dir: Verzeichnis für Snapshots (optional, Standard: <repo>/data/snapshots)
    ...
    if snapshots_dir is None:
        ...
        snapshots_dir = Path(__file__).parents[4] / "data" / "snapshots"
