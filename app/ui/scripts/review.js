 ...
 */
/**
 * Konfidenz als Ring statt als Balken (Detailansicht).
 *
 * Der Balken im LISTEN-Eintrag bleibt, wie er ist — dort stehen viele Vorschläge
 * untereinander, und ein Balken lässt sich zeilenweise vergleichen. In der Detailansicht
 * gibt es nur EINE Konfidenz, und die soll die Seitenspalte anführen.
 *
 * Die Farbschwellen sind dieselben wie beim Balken (`confidenceBar`), damit dieselbe Zahl
 * nicht an zwei Stellen verschieden eingefärbt wird.
 */
function confidenceRing(score) {
    if (typeof score !== 'number') {
        return '<div class="rb-conf-ring rb-conf-ring-empty"><span class="rb-muted">n/a</span></div>';
    }
    const v = Math.max(0, Math.min(1, score));
    const r = 42, c = 2 * Math.PI * r;
    const level = v >= 0.8 ? 'high' : (v >= 0.5 ? 'mid' : 'low');
    return `
        <div class="rb-conf-ring ${level}" role="img"
             aria-label="Konfidenz ${Math.round(v * 100)} Prozent">
            <svg width="96" height="96" viewBox="0 0 96 96" aria-hidden="true">
                <circle cx="48" cy="48" r="${r}" fill="none" stroke="var(--border-color)" stroke-width="9"/>
                <circle cx="48" cy="48" r="${r}" fill="none" stroke-width="9" stroke-linecap="round"
                        class="rb-conf-ring-arc"
                        stroke-dasharray="${c.toFixed(1)}" stroke-dashoffset="${(c * (1 - v)).toFixed(1)}"/>
            </svg>
            <span class="rb-conf-ring-pct">
                <b>${Math.round(v * 100)}%</b><small>Konfidenz</small>
            </span>
        </div>`;
}
...
/**
 * Ladeplatzhalter statt Kreisel: er hat die FORM der spaeteren Karten, dadurch springt
 * das Layout beim Eintreffen der Daten nicht. Der Text bleibt fuer Screenreader stehen —
 * ein Schimmern allein sagt einem Vorleseprogramm nichts.
 */
function renderLoading() {
    const card = `
        <div class="sk-card" aria-hidden="true">
            <div class="sk-row">
                <span class="sk" style="height:22px;width:150px;border-radius:999px"></span>
                <span class="sk" style="height:22px;width:110px;border-radius:999px;margin-left:auto"></span>
            </div>
            <span class="sk" style="height:12px;width:64%"></span>
            <div class="sk-row" style="padding:0">
                <span class="sk" style="height:12px;width:30%"></span>
                <span class="sk" style="height:12px;width:22%"></span>
                <span class="sk" style="height:12px;width:26%"></span>
            </div>
        </div>`;
    proposalList.innerHTML =
        `<span class="sr-only" role="status">Lade offene Vorschläge…</span>`
        + card.repeat(3);
}
...
function renderDetail(p) {
    ...
    /* Zweispaltig (2026-08-02, auf Wunsch): links WAS geändert werden soll, rechts
       WIE BELASTBAR das ist und die Entscheidung. Vorher stand alles untereinander, und
       die Entscheidungsknöpfe lagen so weit unter der Konfidenz, dass man beim Klicken
       nicht mehr sah, worauf sie sich stützt — genau das soll die Seitenspalte ändern.
       Unter 1100 px fällt das Raster auf eine Spalte zurück (siehe styles.css); die
       Reihenfolge im Markup ist deshalb auch gestapelt sinnvoll: erst der Befund, dann
       die Belastbarkeit, dann die Entscheidung. */
        ...
        <div class="rb-detail-grid">
            <div class="rb-col-main">
                ...
                <div class="rb-detail-section" id="codeContextSection" hidden>
                    <h3>
                        <span class="material-symbols-outlined" aria-hidden="true">data_object</span>
                        Fehlerstelle im Original (<code>snapshot-data.json</code>)
                    </h3>
    ...
    return `
        <div class="rb-detail-section rb-conf-card">
            <div class="rb-conf-head">
                ${confidenceRing(p.confidence_score)}
                <div class="rb-conf-head-text">
                    <b>Woher kommt die Zahl?</b>
                    Aus Fehlerklassifikation, Belegprüfung und Gedächtnis früherer Entscheidungen.
                </div>
            </div>
...
function wireDecisionPanel(p) {
    ...
    document.getElementById('modifySubmitBtn').addEventListener('click', () => {
        ...
        armDecision(p.proposal_id, 'modify', parsed);
    });
}
...
/**
 * Gegenstueck zu `lockDecisionPanel` — gebraucht, wenn das Wartefenster abgebrochen wird.
 * Nach einer AUSGEFUEHRTEN Entscheidung wird das nie aufgerufen: dort bleibt gesperrt.
 */
function unlockDecisionPanel() {
    decisionButtons().forEach(b => {
        b.disabled = false;
        b.classList.remove('busy');
    });
 ...
 */
/**
 * „Rückgängig" bei einer Entscheidung — als WARTEFENSTER, nicht als Rücknahme.
 *
 * Warum nicht nachträglich zurücknehmen? Eine Freigabe schreibt drei Dinge: den Review-
 * Eintrag in der Datenbank, einen Eintrag ins Gedächtnis, und die Korrektur in die
 * Snapshot-Datei (samt Revalidierung). Ein Knopf, der das hinterher „zurücknimmt", müsste
 * den Snapshot rückwärts schreiben und einen Gedächtniseintrag tilgen. Beides wäre eine
 * Fälschung der Historie — und das Gedächtnis ist in diesem System die Instanz, an der
 * spätere Vorschläge gemessen werden. Ein falscher Eintrag dort hat uns schon einmal einen
 * richtigen Vorschlag dauerhaft blockiert.
 *
 * Deshalb passiert die Entscheidung erst NACH dem Wartefenster. Wer „Rückgängig" drückt,
 * nimmt nichts zurück — es ist schlicht noch nichts passiert. Danach ist sie endgültig,
 * und das sagt die Oberfläche auch.
 */
const UNDO_WINDOW_MS = 6000;

/** Läuft gerade eine Entscheidung im Wartefenster? Dann keine zweite starten. */
let pendingDecision = null;

function armDecision(proposalId, decision, finalValue) {
    if (pendingDecision) return;

    // Ablehnen ohne Kommentar würde der Server mit 400 zurückweisen — das soll SOFORT
    // auffallen und nicht erst sechs Sekunden später.
    const commentEl = document.getElementById('rbComment');
    if (decision === 'reject' && !(commentEl ? commentEl.value : '').trim()) {
        showDecisionStatus('error', 'Für eine Ablehnung ist ein Kommentar erforderlich.');
        return;
    }

    lockDecisionPanel();
    const label = { approve: 'Freigabe', modify: 'Änderung', reject: 'Ablehnung' }[decision];
    showDecisionStatus('info',
        `<strong>${label}</strong> wird in wenigen Sekunden ausgeführt —
         solange lässt sie sich noch abbrechen.`);

    let cancelled = false;
    const timer = setTimeout(() => {
        pendingDecision = null;
        if (!cancelled) submitDecision(proposalId, decision, finalValue);
    }, UNDO_WINDOW_MS);

    const dismiss = window.AppShell && window.AppShell.toastAction
        ? window.AppShell.toastAction(
            `${label} wird ausgeführt…`, 'hourglass_top', 'Rückgängig', () => {
                cancelled = true;
                clearTimeout(timer);
                pendingDecision = null;
                unlockDecisionPanel();
                showDecisionStatus('info', 'Abgebrochen — es wurde nichts angewendet.');
                if (window.AppShell.toast) window.AppShell.toast('Abgebrochen', 'undo');
            })
        : null;

    pendingDecision = { timer, dismiss };
}
...
async function submitDecision(proposalId, decision, finalValue) {
    ...
    try {
        ...
        if (response.ok) {
            listStale = true;
            lockDecisionPanel();
            /* Kurzmeldung ZUSAETZLICH zum Panel-Text: der Panel-Text ist die Begruendung
               und bleibt stehen, die Meldung ist die Quittung und geht wieder weg.
               BEWUSST OHNE „Rueckgaengig": eine Entscheidung ist in der Datenbank und im
               Gedaechtnis gelandet und hat den Snapshot veraendert. Ein Knopf, der so tut,
               als liesse sich das mit einem Klick zuruecknehmen, waere eine Luege ueber den
               Zustand des Systems — und das Gedaechtnis ist hier die Wahrheitsinstanz. */
            if (window.AppShell && window.AppShell.toast) {
                window.AppShell.toast(
                    decision === 'reject' ? 'Vorschlag abgelehnt'
                    : decision === 'modify' ? 'Geänderter Wert übernommen'
                    : 'Vorschlag genehmigt und angewendet',
                    decision === 'reject' ? 'cancel' : 'check_circle');
            }
            if (decision === 'reject') {
...
proposalList.addEventListener('click', async (e) => {
    ...
    if (copyBtn) {
        ...
        try {
            ...
            if (window.AppShell && window.AppShell.toast) {
                window.AppShell.toast('Snapshot-ID kopiert', 'content_copy');
            }
