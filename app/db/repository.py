...
def list_sessions_as_dicts(limit: int = 50) -> list[dict]:
    ...
    with session_scope() as db:
        ...
        out = []
        for s in rows:
            # Ausgeblendete Sessions: nur aus der LISTE raus. Ihre Nachrichten und
            # Agent-Laeufe bleiben in der Datenbank, sonst wuerden sich die
            # Dashboard-Kennzahlen rueckwirkend aendern.
            ...
            out.append(
                {
                    "session_id": s.id,
                    # Eigener Titel schlaegt den abgeleiteten — aber nur, wenn er
                    # wirklich etwas enthaelt.
                    "title": (s.title or "").strip() or _derive_title(first_user),
...
def rename_session(session_id: int, title: Optional[str]) -> bool:
    """
    Eigenen Titel setzen oder (bei leerer Eingabe) wieder auf den abgeleiteten zurueckfallen.

    Gibt False zurueck, wenn es die Session nicht gibt — der Aufrufer macht daraus ein 404
    statt einer stillen 200, die nichts getan hat.
    """
    clean = (title or "").strip()[:255] or None
    with session_scope() as db:
...
def set_session_hidden(session_id: int, hidden: bool) -> bool:
    """
    Aus der Verlaufsliste nehmen oder zurueckholen. LOESCHT NICHTS.

    Bewusst kein echtes DELETE: `Session.messages` und `Session.agent_runs` haengen an einer
    `delete-orphan`-Cascade, und `agent_runs` ist die Datenquelle fuer Tokens, Kosten und
    Validierungen im Dashboard. Ein Loeschen wuerde die Projektkennzahlen rueckwirkend
    veraendern, ohne dass es jemand bemerkt.
    """
    with session_scope() as db:
        row = db.get(models.Session, session_id)
        if row is None:
            return False
        row.hidden_at = _dt.datetime.now(_dt.timezone.utc) if hidden else None
        return True
