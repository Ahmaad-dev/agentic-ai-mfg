...
class Session(Base):
    ...
    #: Vom Menschen vergebener Titel. Bleibt er leer, gilt der aus der ersten Nachricht
    #: abgeleitete Titel (`_derive_title`) — so behalten alle Alt-Sessions ihr Verhalten.
    title: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)

    #: Ausgeblendet statt geloescht. Ein echtes DELETE wuerde ueber die Cascade unten auch
    #: `agent_runs` mitnehmen, und daraus rechnet das Dashboard Tokens, Kosten und
    #: Validierungen — eine geloeschte Unterhaltung wuerde die Projektkennzahlen rueckwirkend
    #: veraendern. Ausgeblendete Sessions verschwinden nur aus der Verlaufsliste.
    hidden_at: Mapped[Optional[_dt.datetime]] = mapped_column(
