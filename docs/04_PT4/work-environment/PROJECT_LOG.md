# PT4 — Project Log (ABGESCHLOSSEN)

> **Dieses Protokoll ist geschlossen (Stand 2026-08-15).** Es dokumentiert das Praxisprojekt
> PT4 vom 08.07. bis 15.08.2026 und wird nicht mehr fortgeschrieben.
>
> **Neue Einträge gehören nach `docs/BA_PROJECT_LOG.md`** (Bachelorarbeit).
>
> **Warum getrennt und nicht weitergeschrieben:** PT4 und die Bachelorarbeit sind zwei
> Vorhaben. Dieses Protokoll ist der Nachweis für PT4 — jede Zahl im Projektbericht lässt
> sich hier zurückverfolgen. Würden Einträge der Arbeit dazwischenlaufen, wäre weder das eine
> noch das andere sauber belegbar, und es entstünde ein Eigenplagiats-Risiko (siehe
> `BACHELORARBEIT_UMSETZUNGSPLAN.md`, Kapitel 6).
...
### 2026-07-13 — Architekturdiagramme und Berichtsmaterial zum PT4-Cut
- **Status:** done
- **Changed files:** neu `docs/ARCHITEKTURDIAGRAMME_PROJEKTBERICHT.md`, `docs/PROJECT_LOG.md`.
- **What was done:** Plan, vollständiges Projektlog, vorhandene AP5/AP6-Dokumentation, aktuelle Projektstruktur und die realen Aufrufpfade von Web-UI, Orchestrator, Agenten, Smart-Planning-Pipeline, HitL-Review, Repository, Memory/Rulebook, MCP/E-Mail, Dashboard und Deployment wurden zu einer abgabefähigen Architekturdokumentation verdichtet. Enthalten sind 16 nummerierte Architektursichten beziehungsweise 18 Mermaid-Diagramme: C4-Kontext/Container/Komponenten, Chat- und Review-Sequenzen, Proposal- und Draft-Zustände, ER-Modell, Memory und Confidence, MCP-Istgrenze, Dashboard, Deployment, Trust Boundaries, CI/CD, MCP-Zielbild und AP-Traceability; ergänzt um Bildunterschriften, Berichtsgliederung, Formulierungsvorschläge, Grenzen und Literaturbezug zu ISO 42010, C4, arc42, MCP, CoALA, NIST und Human-AI-Interaction. Historische Aussagen aus `README-PT4.md` und `AP5_AP6_DOCUMENTATION.md` wurden nicht ungeprüft als Ist-Stand übernommen; automatische Proposal-E-Mail bleibt korrekt als deaktiviert dokumentiert, AP-E/AP-X als offen beziehungsweise geparkt.
- **Verification:** neue Datei als UTF-8 gelesen: 1.202 Zeilen, 54.948 Zeichen, 18 Mermaid-Blöcke und 19 vollständig gepaarte Codeblöcke; alle fünf relativen Markdown-Links lösen auf; `git diff --check` ohne Fehler. Architekturbeziehungen wurden gegen aktuelle Funktionen/Imports und die GitHub-Workflows geprüft. Kein Runtime-Tool und keine Produkt-/Fixture-Datei geändert. Geschützter Haupt-Snapshot weiterhin SHA-256 `62279F6153EA41216312958F3A3D324ED4E62AB09489874F3980F3A9E61F9414`; `iteration-5/snapshot-data.json` weiterhin `7174A169F72EE3EEF18EFEAD2596BB3463BD432D2E5DD74D57DA541934CDC65C`.
- **Open / next:** Für die Abgabe die empfohlenen 6–8 Hauptdiagramme in den Bericht übernehmen und als SVG exportieren; AP-E/AP-X später getrennt fortsetzen. Das MCP-Zielbild ausdrücklich als Ausblick, nicht als Ist-Implementierung beschriften.


OFFENER PUNKT UND SOGAR EIN SEHR WICHTIGER: FINALISIERUNG ALLER RESSOURCEN UND DEPLOYMENTS AUF TERRAFORM IN DER CLOUD!!!!
SOWIE DIE SICHERSTELLUNG EINER VERBINDUNG ZWISCHEN MEINEM AGENTEN UND DER VM VON ESAROM TEST VM2

---

### 2026-07-14 — AP1–AP7/AP-E-Belegübersicht als Markdown
- **Status:** done
- **Changed files:** neu `docs/AP1_AP7_APE_BELEGE.md`, `docs/PROJECT_LOG.md`.
- **What was done:** Die read-only gegen aktuellen Code, SQLite-Datenbank und Projektlog geprüfte Belegantwort wurde als eigenständige Markdown-Datei übernommen. Enthalten sind reales CorrectionProposal-Schema und Confidence-Versionen, alle ORM-Tabellen, Review-Endpunkte und Apply-Guards, AP4-UI-Beleg, zwölf MCP-Tools und E-Mail-Zustände, sämtliche Dashboard-KPIs und zwölf Data-Quality-Flags, reale Memory-Retrieval-Logik samt Fallbeispiel sowie Screenshot-Inventar, Apply-Messung und ehrliche AP-E-Abgrenzung.
- **Verification:** UTF-8-Datei gelesen: 213 Zeilen, 12.352 Zeichen, AP1 bis AP7 sowie AP-E vollständig vorhanden; beide Codeblöcke geschlossen; `git diff --check` ohne Fehler. Kein Runtime-Tool und keine Produkt-/Fixture-Datei geändert. Geschützter Haupt-Snapshot weiterhin SHA-256 `62279F6153EA41216312958F3A3D324ED4E62AB09489874F3980F3A9E61F9414`; `iteration-5/snapshot-data.json` weiterhin `7174A169F72EE3EEF18EFEAD2596BB3463BD432D2E5DD74D57DA541934CDC65C`.
- **Open / next:** Dashboard-Screenshot für die Projektdokumentation noch aufnehmen; AP-E-Seeding/Acceptance später finalisieren. Die unmittelbar davor vom Nutzer ergänzten offenen Punkte zu Terraform-Cloudressourcen und zur Verbindung mit der Esarom-Test-VM2 bleiben unverändert offen.

---

### 2026-07-31 — AP-E: Isolierte Fehler-Suite gefahren (10 Fälle, Nutzer-Testkatalog)
- **Status:** done (Auswertung) — deckt einen strukturellen Befund + eine Fähigkeitslücke auf
- **Changed files:** `demo/eval/run_isolated_suite.py` (neu). Genutzt: der vom Nutzer gebaute Katalog `smart-planning/Snapshots/pt4-manipulated_snapshots/isolated-error-snapshots/` (10 Snapshots à ~5,7 MB, jeder = ok-snapshot.json mit EINER chirurgischen String-Änderung) + `expected-results.json` als maschinenlesbares Oracle. Validierungsweg: Weg 2 (create ohne Crawl + update + trigger), ~12 persistente Eval-Snapshots auf der SP-Testinstanz (Nutzer-Freigabe; nie löschen).
- **Ergebnis:** **Erkennung 10/10** (Server + Tag-Ableitung + Karten-Auswahl funktionieren für alle 10 Validatoren; Skills-Abdeckung durch die 7 neuen Nutzer-Karten vollständig). **Lokalisierung funktional 8/10**, **Wert exakt 4/10** (I01, I02, I04, I07 — I07 war ein Vergleicher-Artefakt: korrektes vollständiges workItemConfigs-Array in richtiger Prozessreihenfolge).
- **KERNBEFUND — „valide, aber fachlich falsch" ist strukturell und wird hier erstmals exakt messbar:** Bei Fehlern, die den Originalwert ZERSTÖREN (I03 Dichte 1.017→0, I05/I06 departmentId→""/"DEPT_NOT_FOUND", I08 HE01-Zeit 120/0.3→0/0), existiert der wahre Wert nur noch in der ok-Referenz. Das Tool rät einen plausiblen Nachbarwert (I03: 1.14 von Artikel 122600 gleicher Abteilung; I05/I06: 20200 statt 20100; I08: 200/0.25 statt 120/0.3), der die Re-Validierung BESTEHT, aber fachlich falsch ist (Dichte ist eine physikalische Artikelkonstante; „gleiche Abteilung" ≠ „gleiche Dichte"). Bei Fehlern, wo der Wert im Snapshot ÜBERLEBT (I01 Duplikat, I02/I04 Referenz-Integrität), korrigiert das Tool EXAKT — inkl. des Leerzeichen-Werts `SP10        SP01`.
- **FÄHIGKEITSLÜCKE — I09/I10 nicht lokalisierbar:** Fehler außerhalb von `articles`/`demands` (I09 `equipment[].workItems[0]`, I10 `packagingEquipmentCompatibility[].predecessors`) liefern GAR KEINEN Vorschlag (`target_path=None`). Die Pipeline (`identify_snapshot`/`generate_correction`) ist artikel-/bedarf-zentriert.
- **Zwei Verbesserungshebel (nicht gebaut, Nutzerentscheidung offen):** (1) **Referenzdatenquelle** — der Generierungs-Prompt hat einen `reference_data`-Mechanismus; die ok-Referenz (bzw. „letzter gültiger Snapshot") als Referenz einspeisen würde exakte Wiederherstellung zerstörter Werte ermöglichen (I03/I05/I06/I08/I10). (2) **Target-Path-Handling** für `equipment`- und `packaging`-Arrays erweitern (I09/I10). Ohne (1) bleibt bei zerstörten Werten nur „valide, aber geraten".
- **Bedeutung für die Bachelorarbeit:** Der isolierte Katalog mit exakter Ground Truth löst genau das Messproblem, das das Exposé als offen benennt — er trennt „server-valide" von „exakt richtig" und macht die Halluzinationsklasse quantifizierbar. Details je Fall in `pt4-eval-results.json`.
- **Open / next:** Referenzdaten-Entscheidung; danach ggf. Re-Run mit Referenz; dann die 10 kombinierten Fehler-Snapshots (Wiedererkennung/Lernen aus vorherigen Korrekturen). Der `value_ok`-Vergleicher in der Harness ist bei Array-Fixes (I07) und nested-Pfaden (I08) zu streng — für die kombinierte Suite verfeinern.

---

### 2026-07-31 — AP7 Präzisierung: Gedächtnis matcht jetzt auf die konkrete Entität (kein Referenz-Snapshot)
- **Status:** done
- **Changed files:** `demo/db/models.py` (+`memory_items.affected_entity_id`), `demo/alembic/versions/57dbefe2d996_*.py` (Migration, angewendet), `demo/db/repository.py` (`list_memory_items_as_dicts` liefert `affected_entity_id`, neuer Setter), `demo/memory/long_term.py` (`entity_key`, schreibt `affected_entity_id`, `backfill_entity_ids`), `demo/memory/retrieval.py` (entity-präzises Matching), `demo/smart-planning/runtime/generate_correction_llm.py` (ermittelt aktuelle Entität aus `search_results[0].original_object`, reicht sie ins Retrieval).
- **Nutzerentscheidung (wichtig, dokumentiert):** **KEIN Referenz-Snapshot.** Bei zerstörten Werten (Dichte→0 etc.) wüsste auch ein Mensch den Originalwert nicht — die Schätzung aus vergleichbaren Artikeln (z.B. 1.14) ist eine legitime erste Antwort. Die **Autorität ist das episodische Gedächtnis (AP7)**: Der Mensch überstimmt im Review-Dashboard (`modify` → 1.017), das landet in `memory_items`, und beim nächsten Vorkommen DESSELBEN Fehlers schlägt das System den korrigierten Wert vor. Genau der AP7-Loop.
- **Die Präzisierung (das eigentliche Paket):** Das Retrieval matchte bisher nur über `affected_entity_pattern` (`articles[].relDensityMin`) — die konkrete `articleId` war weder im Schlüssel noch in der Tabelle. Folge: eine Korrektur an Artikel 100005 (Dichte 1.017) wäre als „bewährter Wert" auch für JEDEN anderen Artikel mit Dichte-Fehler angeboten worden — das Gedächtnis hätte eine Halluzination EINGEFÜHRT (Dichte ist laut ESAROM-Doku eine artikelspezifische physikalische Konstante). Fix: `affected_entity_id` (kanonisch `typ:id`, z.B. `articles:100005`, aus dem bereits vorhandenen `target_entity_id` von AP3.5a) wird bei jedem Fall gespeichert. **Universelle Regel:** Ein vergangener WERT ist nur autoritativ (`memory_support` 1.0 bzw. negativ 0.0), wenn es DIESELBE Entität ist. Gleiches Muster bei ANDERER Entität → max. 0.5 und im Prompt als `[ANDERES OBJEKT] — nur Methoden-Hinweis, Wert NICHT übernehmen` gekennzeichnet (vs. `[GLEICHES OBJEKT] — verbindlich`).
- **Verifikation:** Unit-Test: Mensch korrigierte 100005-Dichte auf 1.017 → Wiederholung SELBER Artikel, KI schlägt 1.017 vor → `memory_support=1.0`; ANDERER Artikel, gleicher Wert → `0.5` (Wert nicht autoritativ). `current_entity_key` löst aus `original_object` korrekt auf (`articles:100005`, `demands:D100079_001`). Retrieval gegen echte DB: Legacy-Fall #2 (Dichte, `entity_id=None`) wird korrekt NICHT als gleiches Objekt gewertet (degradiert sauber auf Muster-only). `generate` importiert fehlerfrei. Backfill: 2 von 6 Altfällen bekamen `affected_entity_id` (die mit modernem Proposal); die 4 Legacy-Fälle bleiben `None` (rückwärtskompatibel).
- **Open / next:** Seeding-Lauf — der Nutzer reviewt die 10 Vorschläge der isolierten Suite im Dashboard und korrigiert die Schätzungen (I03 1.14→1.017, I05/I06 →20100, I08 →120/0.3). Danach Nachweis des Loops: denselben Fehler erneut fahren → System schlägt jetzt den erinnerten Wert vor. Die `modify`-Entscheidungen trifft der Nutzer (Fall-#7-Lehre: nie fabrizieren).

---

### 2026-07-31 — Seeding-Review durchgeführt: Gedächtnis-Loop bewiesen + 3 Bugs gefixt
- **Status:** done (Kern-Loop bewiesen); UI-/Harness-Bugs gefixt
- **Changed files:** `demo/memory/retrieval.py` (`same_entity_confirmed_value`), `demo/smart-planning/runtime/generate_correction_llm.py` (Re-Retrieval + deterministischer Memory-Override), `demo/ui/scripts/review.js` (Diff-Rendering für Objekt-Werte), `demo/eval/run_isolated_suite.py` (metadata.txt). DB-Cleanup: 7 alte Katalog-Vorschläge aus dem Board entfernt (Gedächtnis unangetastet).
- **HAUPTERGEBNIS — der Gedächtnis-Loop ist bewiesen:** Der Nutzer hat die 10 Vorschläge der isolierten Suite reviewt und die Schätzungen korrigiert (Dichte 1.14→1.017 für Artikel 100005, Department 20200→20100). Diese Modifies landeten trotz „Apply failed" korrekt in `memory_items` — entitäts-präzise (entity=articles:100005). Re-Run desselben Dichte-Fehlers → System schlägt jetzt **1.017** vor, `memory_support=1.0` („Ein Mensch hat genau diesen Wert für DIESES Objekt bereits bestätigt"). Vorher 1.14/0.0.
- **BUG 1 (kritisch) — falscher Retrieval-Schlüssel:** Das Gedächtnis wurde über `search_results[0].path` angesteuert — das ist aber nur der SUCH-ANKER. Bei einem Dichte-Fehler ankert die value-Suche auf der articleId → `results[0].path = articles[].articleId`, NICHT `relDensityMin`. Folge: `memory_support=0.0` trotz vorhandener Fälle. **Fix:** Nach der Generierung ist der echte `target_path` + die Entität (aus `derive_correction_identity`) bekannt → Re-Retrieval damit. Zusätzlich **deterministischer Override:** existiert ein vom Menschen bestätigter Wert für DASSELBE Objekt+Feld, gewinnt er über die frische Schätzung (`value_source=memory`, Begründung mit `[GEDÄCHTNIS]`-Präfix). Das ist die robuste, deterministische Realisierung des Nutzer-Loops.
- **BUG 2 (nutzer-verwirrend) — „Apply pipeline failed at update_snapshot":** Root Cause `FileNotFoundError: metadata.txt not found`. Meine Eval-Harness legte die Snapshots ohne `metadata.txt` an, die `update_snapshot.py` für den Rückschreib-Schritt braucht. **KEIN System-Bug** (echte Snapshots haben sie), und **der Gedächtnis-Loop ist unberührt** (Entscheidung + Memory werden vor dem Apply geschrieben). Harness gefixt (schreibt metadata.txt). Die bestehenden Eval-Snapshots bräuchten ein Backfill, falls der Rückschreib-Schritt für sie nötig wird — für die Evaluation nicht erforderlich.
- **BUG 3 (UI) — „[object Object]" im Diff:** `cell()` machte `escapeHtml(v)` — bei einem einzelnen Objekt-Wert (z.B. I08 HE01-Objekt `{workItemKey, rampUpTime, netTimeFactor}`) ergab das „[object Object]", und der Wert fiel in den Skalar-Zweig (falsches Rot-Highlighting des ganzen Arrays). **Fix:** `cell()` serialisiert Objekte/Arrays; `renderDiff` bekommt einen eigenen Objekt-Zweig (Vorher/Nachher als formatiertes JSON).
- **Fachliche Beobachtungen des Nutzers (bestätigt, kein Bug):** I07 (RF01) — das System hat das VOLLE korrekte Array in richtiger Prozessreihenfolge vorgeschlagen und RF01 wiederhergestellt (Erfolg, nur durch Bug 3 unleserlich dargestellt). I08 (HE01-Zeit) und I05/I06 (Department) — zerstörte Werte, das System schätzt (200/0.25 bzw. 20200); der Mensch korrigiert (120/0.3 bzw. 20100) → Gedächtnis lernt es → nächstes Mal korrekt. Genau der beabsichtigte Loop. I05/I06-Frage des Nutzers („habe ich eine Regel dafür?"): Nein — 20100 ist ein zerstörter Wert, nicht aus dem Snapshot ableitbar; die Autorität ist die menschliche Korrektur im Gedächtnis (siehe Memory `correction-authority-memory-not-reference`).
- **Open / next:** Optional die Eval-Snapshots mit metadata.txt nachrüsten, falls der volle Apply→Server-Loop demonstriert werden soll. Kombinierte Fehler-Suite (10 Snapshots) steht noch aus.

---

### 2026-07-31 — AP-E: Kombinierte Suite gefahren — Gedächtnis-Wiedererkennung bewiesen
- **Status:** done · **Changed files:** `demo/eval/run_combined_suite.py` (neu). ~11 persistente Eval-Snapshots auf der SP-Testinstanz (Weg 2, Nutzer-Freigabe).
- **Aufbau:** Die kombinierte Suite ist ein Curriculum (E01–E12 über 10 Snapshots, Reihenfolge 01→10). Entscheidend: das isolierte Review des Nutzers hat dem Gedächtnis E01 (Duplikat demandId → D100005_002), E02 (Artikel → 100005), E03 (Dichte → 1.017), E04 (Arbeitsplan → SP10 SP01) bereits beigebracht — alle mit denselben Entitäten (Kopien von ok-snapshot). Die kombinierten Snapshots 04/05/08/09 enthalten genau diese bekannten Fehler.
- **Ergebnis:**

  | Datei | erwartet | erkannt | alle | memory_support | Situation |
  |---|---|---|---|---|---|
  | 01/02/03 | 1 | 1 | JA | **1.0** | E01/E02/E03 einzeln — wiedererkannt |
  | 04 | 2 | 2 | JA | **1.0** | E01+E02 bekannt |
  | 05 | 2 | 2 | JA | **1.0** | E03+E04 bekannt |
  | 06 | 2 | 0 | NEIN | 0.0 | siehe Befund unten |
  | 07 | 2 | 2 | JA | 0.0 | 2 neu (Equipment/Qualifikation) |
  | 08 | 3 | 3 | JA | **1.0** | E01+E02+E03 bekannt |
  | 09 | 3 | 3 | JA | **1.0** | E04 bekannt + 2 neu |
  | 10 | 3 | 3 | JA | 0.5 | leere demandId (Typ bekannt, andere Entität) |

  **Erkennung 18/20 Fehler, 9/10 Snapshots vollständig. Gedächtnis-Wiedererkennung (memory_support=1.0) bei 7/10 Snapshots** — die im isolierten Review gelernten Korrekturen E01–E04 werden über die kombinierten Snapshots hinweg wiedererkannt. Das ist der empirische Beleg für die „Lernen aus vorherigen Korrekturen"-Dimension.
- **BEFUND error-06 (kein System-Fehler, Erkenntnis über die Suite):** Die leere `equipmentId` (`""`) erzeugt einen INTEGRITÄTS-/Schemafehler („Input should be a valid UUID", HTTP 463), keinen Inhalts-Validator. Laut ESAROM-Doku §3.1 läuft die Integritätsprüfung ZUERST — schlägt sie fehl, laufen die Inhalts-Validatoren nicht, und der co-injizierte Fehler (Packaging 99999) wird maskiert. Die Suite erwartete `validate_unique_ids`; der echte Server behandelt ein leeres UUID-Feld als Schema-Verstoß. Für die Auswertung: E06 ist als Integritätsfehler zu führen, nicht als Inhaltsfehler.
- **Grenze (erwartet, wie isolierte I09/I10):** Die NEUEN Equipment-/Packaging-Fehler (07, Teile von 09/10) liegen in Ziel-Pfaden außerhalb `articles`/`demands` — memory_support 0.0/0.5, und die Pipeline lokalisiert sie schwächer. Bekannte Pipeline-Lücke.
- **Fazit AP-E-Suiten:** Erkennung stark (isoliert 10/10, kombiniert 18/20). Exakte Wertkorrektur stark, wo der Wert überlebt oder im Gedächtnis steht; Schätzung (valide, aber ggf. fachlich falsch) bei zerstörten Werten ohne Gedächtnis — durch den HitL-Gedächtnis-Loop nachweislich behebbar (bewiesen: Dichte 1.14→1.017 nach Review). Lücken: Equipment/Packaging-Pfade. Alles in `pt4-eval-results.json` / `pt4-combined-results.json`.
- **Open / next:** Optional Equipment/Packaging-Pfade in identify_snapshot/generate erschließen (schließt I09/I10 + error-07). Sonst ist die AP-E-Empirie damit belastbar dokumentiert.

---

### 2026-07-31 — Verbesserung #1: Equipment/Packaging-Lokalisierung (WORK_ITEM_EQUIPMENT_AVAILABILITY)
- **Status:** done
- **Changed files:** `demo/smart-planning/runtime/identify_snapshot.py` (Anomalie-Finder + neuer Modus), `demo/smart-planning/runtime/identify_error_llm.py` (Modus-Override + Trigger). Alles ADDITIV — der bestehende value/empty-Pfad ist unberührt.
- **Präziser Befund (die Eval-Schlagzeile war zu pauschal):** Equipment-Felder werden sehr wohl angesteuert, wenn die Fehlermeldung die Entität nennt (error-06 `equipment[].equipmentId`, error-07 `equipment[].qualification`) — deren zerstörte Werte laufen über den Memory-Loop wie die Dichte. Die EINZIGE echte Lokalisierungs-Panne war `validate_work_item_equipment_availability` (I09): Die Meldung nennt einen fehlenden Key (VOAR01), der value-Modus sucht ihn und trifft 423 valide Stellen (Artikel/Arbeitspläne) — nie das kaputte Equipment (dessen Eintrag durch einen Placeholder ersetzt wurde).
- **Fix:** Für diesen Tag routet `identify_error_llm` auf einen neuen Modus `equipment_workitem`. `identify_snapshot.find_equipment_workitem_anomalies` scannt `equipment[].workItems` nach Einträgen AUSSERHALB der gültigen Work-Item-Keys (VALID_WORK_ITEM_KEYS aus KUNDENDOKUMENTATION §5.1) — das ist der Placeholder — und liefert `equipment[i].workItems[j]` als Ziel plus den benötigten Key aus der Meldung.
- **Verifikation:** I09 (Snapshot 482d71b0) — vorher `target_path=None` (kein Vorschlag). Jetzt: **Target `equipment[334].workItems[0]`, New Value `VOAR01`** = exakt die Ground Truth. `value_grounded=0.0` ist hier korrekt/ehrlich (VOAR01 kam in EQUIPMENT-workItems nur bei genau diesem einen, jetzt kaputten Equipment vor; der Wert stammt autoritativ aus der Validator-Meldung, nicht aus Nachbardaten). Regressionstest: Dichte-Fall (value-Modus) unverändert, inkl. Memory-Override 1.14→1.017.
- **Restliche Equipment/Packaging-Fälle:** keine Lokalisierungs-Panne, sondern zerstörte Werte (equipmentId-UUID, qualification, packaging-predecessor) → Schätzung/manual → Memory-Loop, identisch zum Dichte-Muster. I10 (packaging predecessors gelöscht) bleibt ein zerstörter-Wert-Fall.
- **Kosmetik (offen, klein):** Der `- New Value:`-Print in generate steht VOR dem Memory-Override-Schritt und zeigt daher den Vor-Override-Wert; der gespeicherte Wert ist korrekt (value_source=memory). Für die Konsole ggf. den Print nach den Override verschieben.
- **Open / next:** AP-E-Empirie ist damit rund. Verbleibende kleine Hebel: value_grounded für nested/add_to_array; Confidence-Aufwertung bei memory_support=1.0; Hygiene (15 pending, Legacy-memory ohne entity_id).

---

### 2026-07-31 — Kleine Hebel: Confidence-Aufwertung (v4) + Board-Hygiene
- **Status:** done
- **Changed files:** `demo/smart-planning/runtime/generate_correction_llm.py` (Confidence-Boden bei memory_support=1.0, CONFIDENCE_FORMULA_VERSION → v4). DB-Cleanup: 17 offene Eval-Vorschläge + zentrale Records entfernt.
- **Lever A — Confidence-Kalibrierung (v4):** Ein vom Menschen für DIESE Entität explizit bestätigter Wert (memory_support=1.0) ist das stärkste Signal — stärker als value_grounded oder die LLM-Selbsteinschätzung. Bei einem zerstörten Wert (Dichte 1.017, HE01-Zeiten) ist value_grounded per Konstruktion 0 (der Wert steht nicht mehr im Snapshot), was den bestätigten Wert bisher UNTER eine plausible Schätzung zog (0.675 < 0.725) — die Confidence war für genau den sichersten Fall invertiert. Fix: Boden von 0.9, wenn memory_support=1.0. Gewichte unverändert; v3/v4 unterscheiden sich nur für menschlich bestätigte Werte (Kalibrierungskurve auf eine Generation pinnen). Verifiziert: memory_support=1.0/vg=0 → 0.9 (vorher 0.675); memory_support=0.5 → 0.575 (unverändert); normale Schätzung → 0.725 (unverändert).
- **Lever B — Hygiene:** 17 offene Eval-Vorschläge (Artefakte dieser Session) aus dem Board entfernt → **0 offen**. Erhalten: 14 reviews, 14 memory_items (die Lern-Substanz). Gedächtnis-Qualität geprüft: 4 Legacy-Fälle ohne `affected_entity_id` (2 approve, 1 modify, 1 reject aus Pre-AP3.5a) — **harmlos**, da sie ohne Entitäts-Match nie ein autoritativer Wert-Präzedenzfall werden können (degradieren auf Methoden-Hinweis 0.5). Kein Leak-Risiko.
- **Gesamtstand AP-E / Verbesserungen:** Erkennung stark (isoliert 10/10, kombiniert 18/20), exakte Korrektur wo Wert überlebt/im Gedächtnis, Schätzung→HitL-Loop bei zerstörten Werten (bewiesen), I09-Lokalisierung geschlossen, Confidence für bestätigte Werte korrekt kalibriert, Board sauber. Verbleibend nur noch bewusst geparkte Punkte (Bericht-Caveats, value_grounded nested-Feinschliff, AP-X).
- **Open / next:** Nichts Blockierendes. Optionaler Feinschliff: den `- New Value:`-Print in generate hinter den Memory-Override verschieben (Kosmetik); 3 redundante (aber übereinstimmende) Dichte-1.017-Gedächtnisfälle deduplizieren.

---

### 2026-08-01 — Feinschliff-Paket (Punkte #1–#11 aus der Restliste)
- **Status:** done, mit 3 ehrlichen Befunden (siehe unten)
- **Changed files:** `demo/smart-planning/runtime/generate_correction_llm.py` (#5 Print-Reihenfolge, #7 ENTITY_IDENTITY_FIELD erweitert), `demo/memory/retrieval.py` (#7 konsistent), `demo/rulebook_loader.py` (#10 check_card_conflicts + CLI + KNOWN_VALIDATOR_TAGS vervollständigt), `demo/eval/run_iterative.py` (#9 neu). DB: #6 Dedup.
- **#1 value_grounded nested/add_to_array:** bereits in AP-E.0 gebaut (`_grounded_for_new_object`, Listen-Mitgliedschaft) — verifiziert an `equipment[i].predecessors[0]` (belegt→1.0, unbekannt→0.0). Kein Eingriff.
- **#2 HTTP 422 in review.js:** bereits in der Tech-Debt-Runde gebaut. Verifiziert.
- **#3 action-Semantik:** per Konvention geregelt (work-item-configs.md schreibt update_field auf das ganze Array vor; Apply ersetzt korrekt). Kosmetisch, kein Risiko-Eingriff.
- **#4 AP6 Pre-AP3.3d:** VERIFIZIERT bereits ausgeschlossen — `revalidation_success_rate = _rate(reval_success, len(trusted))`, Nenner nur trusted (mit errors_before). Untrusted separat + Flag. Log-Notiz war veraltet.
- **#5 (done):** `- New Value:` heißt jetzt „(LLM roh)"; nach dem Memory-Override kommt „- New Value (final): … (Quelle: memory/llm)". Kein Widerspruch mehr in der Konsole.
- **#6 (done):** Gedächtnis dedupliziert (pro Entität/Muster/Entscheidung/Wert den neuesten behalten): 14→11 Fälle (3 Dichte-1.017-Duplikate + 1 Department-Duplikat entfernt).
- **#7 (done):** ENTITY_IDENTITY_FIELD in generate UND retrieval konsistent um equipment→equipmentId, workerAvailability/workerQualifications→workerId, packagingEquipmentCompatibility→packaging erweitert. Verifiziert: Guard-target_entity_id und Memory-Key stimmen überein (`equipment:uuid-123`), Artikel-Regression ok. Hinweis: nicht gegen reale Sonderentitäten-Daten getestet (keine vorhanden).
- **#10 (done):** `check_card_conflicts()` meldet Tags mit ≥2 Karten (Konfliktverdacht; mehrere Karten pro Tag sind erlaubt, aber prüfenswert). CLI `python rulebook_loader.py`. NEBENFUND & FIX: `KNOWN_VALIDATOR_TAGS` war unvollständig → flaggte 4 legitime Karten (article-departments, equipment-work-items, human-availability, packaging-references) fälschlich als Tippfehler. Um die 5 fehlenden real bestätigten Validatoren ergänzt. Diagnose jetzt: 0 Tippfehler, 0 Tag-Konflikte.
- **#9 (Runner gebaut, Live-Run blockiert):** `eval/run_iterative.py` fährt einen Snapshot iterativ (identify→generate→schema→apply→upload→revalidate) bis 0 Fehler. AUSDRÜCKLICH EVAL-MODUS: Direkt-Apply, KEINE Review-Zeile/Gedächtnisfall (fabriziert keine menschliche Entscheidung; Produktion bleibt HitL). Live-Nachweis auf kombiniert-08 (3 bekannte Fehler) scheiterte an einem **Netzwerk-/DNS-Ausfall der SP-Testinstanz** (`Failed to resolve vm-t-…-test02…`) — kein Code-Fehler; nachholbar, sobald der Server wieder erreichbar ist.
- **#11 (NICHT entfernt — Prämisse falsch):** Der USE_REFERENCE_DATA-Mechanismus ist NICHT tot. `identify-tool-files/reference-snapshot.json` existiert, Config-Default `use_reference_data_fallback=True`. Er bedient den EMPTY-ARRAY-Fall (leeres Top-Level-Array aus der Referenz füllen) — etwas ANDERES als der zurückgezogene „Referenz für zerstörte Skalarwerte". Entfernen würde eine lebende Fähigkeit brechen. Bewusst belassen.
- **#8 (geflaggt, kein Code):** Equipment/Packaging-Fälle mit zerstörtem Wert (I10 packaging-predecessor, equipment-qualification) laufen über den Memory-Loop — brauchen genau EINEN menschlichen Review pro Fall (wie die Dichte), den ich nicht fabrizieren darf. Kein Bug, sondern der gewollte HitL-Ablauf.
- **Open / next:** #9-Live-Nachweis nachholen (Server-Erreichbarkeit); #8 vom Nutzer reviewen lassen, wenn gewünscht.

---

### 2026-08-01 — #9 LIVE bewiesen: iterative Mehrfehler-Korrektur + Memory-Override in einem Lauf
- **Status:** done — der letzte offene Fähigkeitspunkt ist geschlossen
- **Lauf:** `eval/run_iterative.py` auf kombiniert-08 (27414fc9), 3 bekannte Fehler (E01 Duplikat-demandId, E02 unbekannter Artikel, E03 Dichte). Ergebnis: **Fehler-Verlauf 3 → 2 → 1 → 0, VALIDE.**
  - Runde 1: `demands[1].demandId` → `D100005_002` (Referenz-Integrität)
  - Runde 2: `demands[0].articleId` → `100005`
...
## PT4 — ABSCHLUSS (2026-08-15)

Damit endet das Protokoll des Praxisprojekts. Was am Ende steht:

**Abgeschlossen:** M0 bis M7 vollständig (Correction Proposal Layer, Persistenz, HitL-Backend
und -Frontend, MCP, Dashboard, Memory). M8/AP-E steht empirisch (isolierte und kombinierte
Suite, Seeding, A/B) — offen blieben die AK2-Zahl nach dem Lernen und die 11-Schritte-Demo.

**Die letzten beiden Tage** (14./15.08.) waren eine dichte Fehlerbehebungsphase, angestossen
durch Nutzerbefunde im laufenden Betrieb. Ergebnis: 16 Befunde, verdichtet auf sechs
wiederkehrende Muster — dokumentiert in `docs/04_PT4/BEFUNDE_UND_LEHREN.md`. Der Systemzustand ist
in `docs/04_PT4/AGENTEN_ARCHITEKTUR.md` beschrieben, die Konfidenz-Berechnung in
`docs/04_PT4/KONFIDENZ.md`.

**Vier Prüfläufe** sichern das Erreichte gegen Rückschritte ab und laufen gegen das echte
System und das echte Modell: `test_agent_truthfulness.py`, `test_agent_context_access.py`,
`test_agent_architektur.py`, `check_doku_stimmt.py` (alle unter `app/eval/`).

**Bewusst offen gelassen** (in `04_PT4/AGENTEN_ARCHITEKTUR.md` §7 im Einzelnen):
kein automatisches Anbieten des nächsten Fehlers nach einer Freigabe; 429-Kontingentfehler
werden nicht als solche erkannt und die Wiederholung wartet nur eine Sekunde; `localhost`
kostet über IPv6 rund zwei Sekunden je Anfrage.

**Was aus PT4 in die Bachelorarbeit übergeht:** die Fehlerinjektion als Ground-Truth-Methode
(`app/eval/build_test_catalog.py`), der `RULEBOOK_MODE`-Schalter als Teilbaustein und die
deterministische technische Prüfung. Alles Übrige — HitL, Dashboard, MCP, Memory,
Confidence-Scoring — bleibt funktionsfähig, gehört aber nicht in den Architekturvergleich.

Weiter geht es in **`docs/BA_PROJECT_LOG.md`**.
